 ...
 *
 */
public class RandomizedQueue_QiCao<Item> implements Iterable<Item> {
	...
	/**
	   ...
	   public Iterator<Item> iterator(){
		...
		return new CustomIterator(arrayItems,length);
	   ...
	   class CustomIterator implements Iterator<Item>{
		   ...
		   private Item[] intertorQueue;
		   
		   public CustomIterator (Item[] arrayItems,int length) {
			   intertorQueue = (Item[]) new Object[length];
			   
			   for(int i = 0; i<intertorQueue.length;i++) {
				   intertorQueue[i]=arrayItems[i];
			   }
			   
			   for(int j=0; j<intertorQueue.length;j++) {
